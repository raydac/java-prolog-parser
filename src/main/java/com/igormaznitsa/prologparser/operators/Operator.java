...
public final class Operator extends AbstractPrologTerm {
	...
	public Operator(final int priority, final OperatorType type,
		...
		precalculatedHashCode = (name + "!" + this.opType + "!" + this.opPriority)
				.hashCode();
	}
		...
		if (struct != null) {
			...
			case 2:
				...
				case YFX:
					...
					final AbstractPrologTerm elementRight = struct
							.getElement(1);
				...
				case XF:
				case FX: {
					final AbstractPrologTerm atom = struct
							.getElement(opType == OperatorType.XF ? 0 : 1);
					if (atom == null) {
						result = false;
					} else {
						result = atom.getPriority() < getPriority();
				...
				case FY: {
					final AbstractPrologTerm atom = struct
							.getElement(opType == OperatorType.YF ? 0 : 1);
					if (atom == null) {
						result = false;
					} else {
						result = atom.getPriority() <= getPriority();
